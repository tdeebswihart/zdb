...
pub const TuplePage = struct {
    ...
    pub fn init(page: *Page) Error!*Self {
        var self = @ptrCast(*TuplePage, @alignCast(@alignOf(TuplePage), page.buffer));
...
test "pages can be written to" {
    ...
    const mgr = try FileManager.init(&fs.manager, 512, 1024, t.allocator);

    const expected: []const u8 = &[_]u8{ 0x41, 0x42, 0x43 };
    var page = try mgr.pin(0);

    var xPage = Writable.init(page);
    const loc = try xPage.put(expected);
    xPage.deinit();
    var shared = Readable.init(page);
...
test "pinned pages are not evicted" {
    ...
    defer dbfile.close();
    var fs = try File.init(dbfile);
    const mgr = try FileManager.init(&fs.manager, 128, 128, t.allocator);
